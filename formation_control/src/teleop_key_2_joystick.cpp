...
int wait_for_drone(int i, double x_targ, double y_targ, double z_targ)
{
   ...
   // if (angle==1)
//	{
   	if ((norm <1.0) & (fabs(fabs(w_real_pos[i])-fabs(w_targ[i]))<0.2) & (fabs(fabs(zang_real_pos[i])-fabs(zang_targ[i]))<0.2)) // (fabs(w_real_pos[i]-w_real_back[i])<0.0005) & (fabs(zang_real_pos[i]-z_real_back[i])<0.0005) )
    	{return 1;}
    	else
    	{return 0;}
//	}
